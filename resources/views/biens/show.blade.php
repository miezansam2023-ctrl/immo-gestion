...
<div class="p-4 md:p-8 bg-[#F8FAFC] min-h-screen">
    <div class="max-w-7xl mx-auto">
        ...
        <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
            <div>
                <a href="{{ route('biens.index') }}" class="group inline-flex items-center text-sm font-bold text-gray-400 hover:text-indigo-600 transition-colors">
            ...
            <div class="flex items-center gap-3">
                <a href="{{ route('biens.edit', $bien) }}" class="flex items-center bg-white text-[#1E293B] border border-gray-200 px-6 py-3 rounded-2xl font-bold hover:bg-gray-50 transition-all shadow-sm">
    ...
    photos: {{ json_encode(is_array($bien->photos) ? $bien->photos : json_decode($bien->photos, true) ?? []) }} 
                        ...
                        <div class="relative aspect-video overflow-hidden group">
                            ...
                            <div class="absolute top-8 left-8 z-20">
                                <span class="px-6 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest shadow-2xl {{ $bien->statut == 'disponible' ? 'bg-green-500 text-white' : 'bg-orange-500 text-white' }}">
                                    ● {{ $bien->statut }}
                                </span>
                            </div>
                            ...
                            <template x-if="photos.length > 1">
                                <div class="absolute inset-0 flex items-center justify-between px-4 opacity-0 group-hover:opacity-100 transition-opacity">
                                    <button @click="activePhoto = activePhoto === 0 ? photos.length - 1 : activePhoto - 1" class="bg-white/20 backdrop-blur-md hover:bg-white/40 text-white p-3 rounded-full transition-all">
                                        <i class="fas fa-chevron-left"></i>
                                    </button>
                                    <button @click="activePhoto = activePhoto === photos.length - 1 ? 0 : activePhoto + 1" class="bg-white/20 backdrop-blur-md hover:bg-white/40 text-white p-3 rounded-full transition-all">
                            ...
                            <div class="absolute bottom-6 left-1/2 -translate-x-1/2 flex gap-2 z-30">
                                <template x-for="(photo, index) in photos" :key="index">
                                    <button @click="activePhoto = index"
                                        ...
                                        :class="activePhoto === index ? 'border-indigo-500 scale-110' : 'border-white/50 scale-100 hover:scale-105'">
                    ...
                    <div class="p-8 md:p-12">
                        <div class="mb-10">
                            <h1 class="text-4xl font-black text-[#1E293B] mb-2 uppercase italic leading-none">{{ $bien->titre }}</h1>
                        ...
                        <div class="grid grid-cols-2 md:grid-cols-4 gap-8 mb-10">
                            ...
                            <div class="bg-gray-50 p-6 rounded-[2rem] border border-gray-100">
                                ...
                                <p class="font-black text-indigo-600">
                                    {{ number_format($contratActif ? $contratActif->loyer_mensuel : $bien->prix_loyer, 0, ',', ' ') }} <span class="text-[8px]">FCFA</span>
                                </p>
                            </div>
                            <div class="bg-gray-50 p-6 rounded-[2rem] border border-gray-100">
                                <p class="text-[10px] font-black text-gray-400 uppercase mb-2">Pièces</p>
                                <p class="font-black text-[#1E293B]">{{ $bien->nombre_pieces ?? $bien->nb_pieces ?? 'N/A' }}</p>
                            </div>
                        </div>

                        <div class="space-y-4">
                            <h3 class="text-[11px] font-black text-gray-400 uppercase tracking-widest flex items-center">
            ...
            <div class="lg:col-span-4 space-y-6">
                ...
                <div class="bg-[#1E293B] rounded-[2.5rem] p-8 text-white shadow-2xl relative overflow-hidden">
                    <div class="relative z-10">
                        <h3 class="text-indigo-400 text-[10px] font-black uppercase tracking-[0.2em] mb-8">Situation Locative</h3>
                        ...
                        <div class="flex flex-col items-center text-center">
                            <div class="relative mb-6">
                                <div class="h-28 w-28 rounded-full border-4 border-indigo-500/30 p-1">
                                    ...
                                    <img src="{{ asset('storage/' . $contratActif->locataire->photo) }}" class="w-full h-full rounded-full object-cover shadow-2xl">
                                    @else
                                    <div class="w-full h-full bg-indigo-600 rounded-full flex items-center justify-center text-3xl font-black italic">
                                        {{ substr($contratActif->locataire->nom, 0, 1) }}
                                    </div>
                                    @endif
                                </div>
                                <div class="absolute bottom-1 right-1 h-6 w-6 bg-green-500 border-4 border-[#1E293B] rounded-full"></div>
                            </div>

                            <p class="text-xs text-indigo-300 font-black uppercase tracking-tighter mb-1">Locataire Actuel</p>
                            <p class="text-2xl font-black uppercase italic mb-8 tracking-tight">{{ $contratActif->locataire->nom }} {{ $contratActif->locataire->prenoms }}</p>
                            ...
                            <div class="w-full grid grid-cols-1 gap-3 mb-8">
                                <div class="bg-white/5 border border-white/5 p-4 rounded-2xl flex justify-between items-center">
                                    ...
                                    <span class="font-black text-sm">{{ \Carbon\Carbon::parse($contratActif->date_debut)->format('d/m/Y') }}</span>
                                </div>
                                <div class="bg-white/5 border border-white/5 p-4 rounded-2xl flex justify-between items-center">
                                    <span class="text-xs font-bold text-gray-400 uppercase">Loyer mensuel</span>
                                    <span class="font-black text-indigo-400">{{ number_format($contratActif->loyer_mensuel, 0, ',', ' ') }} <span class="text-[10px]">CFA</span></span>
                                </div>
                            </div>

                            <a href="{{ route('contrats.show', $contratActif) }}" class="group w-full bg-indigo-600 hover:bg-indigo-700 py-4 rounded-[1.2rem] font-black transition-all text-[10px] uppercase tracking-widest shadow-xl flex items-center justify-center">
                                VOIR LE CONTRAT
                        ...
                        <div class="text-center py-12">
                            <div class="w-20 h-20 bg-green-500/10 rounded-3xl flex items-center justify-center mx-auto mb-6">
                                <svg xmlns="http://www.w3.org/2000/svg" class="h-10 w-10 text-green-500" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" />
                                </svg>
                            </div>
                            <p class="text-green-400 font-black text-xl italic uppercase">Libre de suite</p>
                            <p class="text-gray-500 text-[10px] font-bold mt-2 uppercase tracking-widest">Aucun contrat actif</p>
                        </div>
                        @endif
                    </div>
                    {{-- Effet de lumière --}}
                ...
                <!-- <div class="bg-white rounded-[2.5rem] p-8 border border-gray-100 shadow-sm">
                    <div class="flex items-center gap-4">
                        ...
                        <div>
                            <p class="text-[10px] font-black text-gray-400 uppercase tracking-widest">Total Encaissé</p>
                            <p class="text-2xl font-black text-[#1E293B] tracking-tight">
                                {{ number_format($bien->paiements()->where('statut', 'paye')->sum('montant_paye'), 0, ',', ' ') }}
                                <span class="text-xs text-gray-400 font-bold uppercase ml-1 italic">FCFA</span>
                            </p>
                        </div>
