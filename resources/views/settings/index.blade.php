...
<div class="max-w-7xl mx-auto px-4 py-10" x-data="{ tab: 'profile' }">
    ...
    <div x-data="{ show: true }" x-show="show" class="relative mb-8 flex items-center p-4 bg-emerald-50 border-l-4 border-emerald-500 rounded-r-2xl shadow-sm">
        ...
        <button @click="show = false" class="ml-auto p-2 group"><svg class="h-5 w-5 text-emerald-300 group-hover:text-emerald-600 transition-colors" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    <div class="mb-10 flex flex-col md:flex-row md:items-start justify-between gap-4">
        ...
        <div class="flex items-center bg-white border border-gray-100 px-4 py-2 rounded-2xl shadow-sm self-start md:self-center">
    ...
    <div class="flex flex-col md:flex-row gap-8">
        ...
        <div class="w-full md:w-72 space-y-2">
            <button @click="tab = 'profile'"
                :class="tab === 'profile' ? 'bg-blue-600 text-white shadow-xl shadow-blue-100' : 'bg-white text-gray-500 hover:bg-gray-50'"
            ...
            <button @click="tab = 'password'"
                :class="tab === 'password' ? 'bg-gray-900 text-white shadow-xl shadow-gray-200' : 'bg-white text-gray-500 hover:bg-gray-50'"
                class="w-full flex items-center px-6 py-4 rounded-[24px] transition-all duration-300 group">
            ...
            <div class="p-6 bg-indigo-50 rounded-[32px] mt-10">
                ...
                <p class="text-[10px] text-indigo-900 font-bold leading-relaxed">Pour toute modification de rôle, contactez l'administrateur système.</p>
            </div>
        </div>

        <div class="flex-1">

            <div x-show="tab === 'profile'" x-transition:enter="transition ease-out duration-300" x-transition:enter-start="opacity-0 translate-y-4">
                <div class="bg-white rounded-[32px] p-8 border border-gray-100 shadow-sm">
                    ...
                    <form action="{{ route('settings.profile.update') }}" method="POST" enctype="multipart/form-data" class="space-y-8">
                        @csrf
                        @method('PUT')

                        <div class="flex flex-col md:flex-row gap-10 items-center md:items-start border-b border-gray-50 pb-10">
                            ...
                            <div class="flex-1 grid grid-cols-1 md:grid-cols-2 gap-6 w-full">
                                ...
                                <div>
                                    <label class="block text-[10px] font-black text-gray-400 uppercase mb-2 ml-1 tracking-widest">Nom</label>
                                ...
                                <div>
                                    <label class="block text-[10px] font-black text-gray-400 uppercase mb-2 ml-1 tracking-widest">Prénoms</label>
                                    <input type="text" name="prenoms" value="{{ old('prenoms', auth()->user()->prenoms) }}"
                                        class="w-full bg-gray-50 border-none rounded-2xl px-5 py-4 text-sm font-bold text-gray-700 focus:ring-2 focus:ring-blue-500 transition-all">
                                </div>
                                <div>
                                    <label class="block text-[10px] font-black text-gray-400 uppercase mb-2 ml-1 tracking-widest">Email professionnel</label>
                        ...
                        <div class="flex justify-end">
                            <button type="submit" class="bg-blue-600 text-white px-10 py-4 rounded-2xl font-black text-[10px] uppercase tracking-widest hover:bg-blue-700 hover:shadow-xl transition-all duration-300">
            ...
            <div x-show="tab === 'password'" x-cloak x-transition:enter="transition ease-out duration-300" x-transition:enter-start="opacity-0 translate-y-4">
                <div class="bg-white rounded-[32px] p-8 border border-gray-100 shadow-sm">
                    ...
                    <!-- derniere modif du mot de passe :  -->
                    <h4 class="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-1 ml-1">Dernière modification</h4>

                    <span class="text-sm font-bold text-gray-500">{{ auth()->user()->password_updated_at ? auth()->user()->password_updated_at->diffForHumans() : 'Jamais' }}</span>
                    ...
                    <form action="{{ route('settings.password.update') }}" method="POST" class="max-w-xl space-y-6">
                        ...
                        <div>
                            <label class="block text-[10px] font-black text-gray-400 uppercase mb-2 ml-1">Mot de passe actuel</label>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 pt-4">
                            <div>
                                <label class="block text-[10px] font-black text-gray-400 uppercase mb-2 ml-1">Nouveau mot de passe</label>
                            ...
                            <div>
                                <label class="block text-[10px] font-black text-gray-400 uppercase mb-2 ml-1">Confirmation</label>
                        ...
                        <div class="flex justify-start mt-10">
                            <button type="submit" class="bg-gray-900 text-white px-10 py-4 rounded-2xl font-black text-[10px] uppercase tracking-widest hover:bg-black hover:shadow-xl transition-all duration-300">
